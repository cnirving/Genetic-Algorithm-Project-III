...
public class Processes {
	...
	public static String[] geneCoding(String geneCoding) {
		...
		case "pendulum":
			...
			chromosome[9] = String.valueOf(1.52);
			chromosome[chromosome.length-1] = String.valueOf(-1.5708);
	...
	public static Population evolveStepOld(Population population) {
		
		Individual[] members = population.getMembers();
		Individual[] newGenerationMembers = new Individual[members.length];
		
		switch (Main.SELECT_TYPE) {
			case "roulette":
				for (int i = 0; i < members.length; i += 2) {
		
					if (i == 0 && Main.ELITISM == true) {
						
						newGenerationMembers[i] = population.getFittest();
						Population tempPop = new Population(Algorithms.removeIndividual(members, newGenerationMembers[i]));
						newGenerationMembers[i + 1] = tempPop.getFittest();
					}
					else {
						
						Individual[] matingPair = Algorithms.rouletteGetPair(population.getMembers());
						Individual[] offspring = crossover(matingPair);
						offspring = mutate(offspring);
						
						newGenerationMembers[i] = offspring[0];
						newGenerationMembers[i + 1] = offspring[1];	
					}
				}
				break;
			...
			case "SRWR":
				...
				int newGenCount = 0; // Keeps track of how many individuals have been added to the new generation
				int newGenNumToFill = newGenerationMembers.length; // Keeps track of how many spaces there are in the new generation to be filled
				// If Elitism is true, fix the first 2 places in the new generation with the 2 fittest from the old generation
				if (Main.ELITISM) {

					newGenerationMembers[newGenCount] = population.getFittest();
					Population tempPop = new Population(Algorithms.removeIndividual(members, newGenerationMembers[newGenCount]));
					newGenerationMembers[newGenCount + 1] = tempPop.getFittest();
					newGenCount += 2;
					newGenNumToFill -= newGenCount; // With elitism on, 2 spaces in the new generation are automatically filled
				}
				
				// Create arrays to hold the (partly determined, partly roulette-selected) mating pool and the (randomly matched) mating pairs
				Individual[] matingPool = new Individual[newGenNumToFill];
				Individual[][] matingPairs = new Individual[newGenNumToFill/2][2];
				double[] expectedValues = new double[newGenNumToFill];
				
				double[] relFitArray = Algorithms.getRelFitArray(members);

				// add individuals to mating pool according to integer part of expected value
				int expInt;
				double[] expDec = new double[newGenNumToFill];
				int poolCount = 0; //keeps track of how many individuals are in the mating pool
				for (int i = 0; i < newGenNumToFill; i++) {
					
					expectedValues[i] = relFitArray[i] * newGenNumToFill;
					...
					for (int j = 0; j < expInt; j++) {
						...
						matingPool[poolCount] = members[i];
						poolCount++;
					}
				}
				
				// add individuals to mating pool according to roulette wheel selection based on decimal part of expected value
				int spaceCount = matingPool.length - poolCount;
				Individual selected;
				for (int i = 0; i < spaceCount; i++) {
					
					selected = Algorithms.rouletteGetSingle(members, expDec);
				...
				for (int i = 0; i < newGenNumToFill / 2; i++) {
		...
		Population newGeneration = new Population(newGenerationMembers);
			...
			case "pendulum":
				...
				if (Main.actionMutate.equals("a little")) {				
					...
					for (int i = 0; i < 2; i++) {
						...
						// First and final genes remain fixed to determine start and end points
						for (int j = 2; j < chromosomes[i].length-1; j++) {
							
							if (Algorithms.coinFlip(Main.MUTATION_RATE) & j!= 9) {
								geneValue = Double.parseDouble(chromosomes[i][j]);
